...
- **`CriteriaOutputFieldKey` derived from the kit's base enum** ([`hear/serializer/model/criteria/output/CriteriaOutputFieldKey.py`](hear/serializer/model/criteria/output/CriteriaOutputFieldKey.py)): the 10 base members (`CREATED_ON`, `UPDATED_ON`, `UUID`, `NAME`, `NAME_INTERNAL`, `ROOT`, `PARENT`, `ASCENDANTS`, `DESCENDANTS`, `CHILDREN`) were hand-copied from `the-music-tree-genre-kit`'s `CriteriaOutputFieldKey`, so a future kit-side addition (e.g. `SIDE`) would silently never propagate here. Since `StrEnum` can't be subclassed once it has members, the local enum is now built via the functional API from the kit's enum members plus hear's own extensions (`UPLOADED_TRACKS_*`, `CRITERIA_PLAYLIST`), instead of duplicating the base member names/values by hand. Pure refactor: every member name and value is unchanged, no new field added.
