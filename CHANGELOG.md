...
- **Pre-commit actionlint**: [`.pre-commit-config.yaml`](.pre-commit-config.yaml) adds **`rhysd/actionlint`** hook **`actionlint-system`** (**`rev: v1.7.12`**) with **`-config-file .github/actionlint.yaml`** on **`.github/workflows/*.yml`**. [**`scripts/install-actionlint.sh`**](scripts/install-actionlint.sh) installs pinned **1.7.12** into **`/usr/local/bin`** on Linux **amd64**/**arm64**; [**`scripts/install-dependencies.sh`**](scripts/install-dependencies.sh) runs it when building the **`api`** image. [**`.github/workflows/test.yml`**](.github/workflows/test.yml) **pre-commit** job and [**`branch-protection.yml`**](.github/workflows/branch-protection.yml) **actionlint** job run **`bash scripts/install-actionlint.sh`** (replacing ad hoc **curl** in branch-protection). [**`scripts/setup-docker-dev-tools.sh`**](scripts/setup-docker-dev-tools.sh) verifies **`actionlint -version`** in the container.
