...
- **Django / pytest startup visibility**: [`api/utils/utils.py`](api/utils/utils.py) **`print_django`** uses **`flush=True`**. [`api/settings.py`](api/settings.py) installs [`api/CiPytestStartupTracer.py`](api/CiPytestStartupTracer.py) for **`apps.populate()`** / **`AppConfig.ready()`** / **`get_resolver()`** in CI/pytest. [`pytest.ini`](pytest.ini) **`addopts = -p api.ci_pytest_startup_plugin`** loads [`api/ci_pytest_startup_plugin.py`](api/ci_pytest_startup_plugin.py) by import path (bind-mounted CI workspaces do not keep image-local **`*.egg-info`**, so **`pytest11`** entry points alone were unreliable). That plugin’s **`pytest_load_initial_conftests`** hookwrapper (**`tryfirst=True`**) prints **outer enter/leave** (stdout + stderr) around capture, pytest-django **`django.setup()`**, and initial conftest import. [`api/urls.py`](api/urls.py) logs URLconf import; [`api/test/conftest.py`](api/test/conftest.py) and [`api/test/tests/conftest.py`](api/test/tests/conftest.py) add **`[pytest]`** progress markers.
