...
- **Deploy workflow**: App env file now sets `SPOTIFY_OAUTH_ENABLED=true`, `GOOGLE_OAUTH_ENABLED=true`, and `MUSICBRAINZ_LOOKUP_ENABLED=true` so Django settings load on deployment (these vars are required, no defaults).
