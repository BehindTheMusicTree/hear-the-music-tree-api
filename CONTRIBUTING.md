...
**actionlint:** [`.pre-commit-config.yaml`](.pre-commit-config.yaml) runs **`actionlint-system`** (**`rhysd/actionlint`** **`v1.7.12`**) with **`-config-file .github/actionlint.yaml`** on **`.github/workflows/*.yml`**, aligned with the **`actionlint`** job in [**`branch-protection.yml`**](.github/workflows/branch-protection.yml) and the **Test** **pre-commit** job (same install script). The **`api`** image installs a pinned Linux binary via [`scripts/install-actionlint.sh`](scripts/install-actionlint.sh) (called from [`scripts/install-dependencies.sh`](scripts/install-dependencies.sh)). If you run **`pre-commit`** on the host, put **`actionlint`** **1.7.12** on **`PATH`** (for example **Homebrew** or a manual download); [`scripts/install-actionlint.sh`](scripts/install-actionlint.sh) targets **Linux** **amd64**/**arm64** (CI and the container).
