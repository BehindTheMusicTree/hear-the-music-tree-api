from api.model.album.Fields import Fields as ModelFields


class Fields:
    CREATED_ON = ModelFields.CREATED_ON
    UPDATED_ON = ModelFields.UPDATED_ON
    UUID = ModelFields.UUID
    NAME_PUBLIC = ModelFields.NAME_PUBLIC
    YEAR = ModelFields.YEAR
    ALBUM_ARTISTS = ModelFields.ALBUM_ARTISTS
    UPLOADED_TRACKS_NOT_ARCHIVED_SORTED_INTERNAL = ModelFields.UPLOADED_TRACKS_NOT_ARCHIVED_SORTED_INTERNAL
    UPLOADED_TRACKS_NOT_ARCHIVED_SORTED_PUBLIC = ModelFields.UPLOADED_TRACKS_NOT_ARCHIVED_SORTED_PUBLIC
    UPLOADED_TRACKS_NOT_ARCHIVED_COUNT_INTERNAL = ModelFields.UPLOADED_TRACKS_NOT_ARCHIVED_COUNT_INTERNAL
    UPLOADED_TRACKS_NOT_ARCHIVED_COUNT_PUBLIC = ModelFields.UPLOADED_TRACKS_NOT_ARCHIVED_COUNT_PUBLIC
    UPLOADED_TRACKS_ARCHIVED_COUNT_INTERNAL = ModelFields.UPLOADED_TRACKS_ARCHIVED_COUNT_INTERNAL
    UPLOADED_TRACKS_ARCHIVED_COUNT_PUBLIC = ModelFields.UPLOADED_TRACKS_ARCHIVED_COUNT_PUBLIC
    DURATION_IN_SEC = ModelFields.DURATION_IN_SEC
    DURATION_STR_IN_HOUR_MIN_SEC = ModelFields.DURATION_STR_IN_HOUR_MIN_SEC
