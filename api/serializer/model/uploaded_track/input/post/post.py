...
class UploadedTrackPostSerializer(UploadedTrackInputSerializer):
    ...
    def _get_metadata_from_file(self, file) -> dict:
        try:
            ...
            raise AppValidationException(field_name=UploadedTrackInputFieldKey.TRACK_FILE_PUBLIC.value,
                                         message=str(exc),
    ...
    def _extract_metadata_fields(self, metadata_dict: dict) -> dict:
        ...
        if metadata_dict.get(AppMetadataKey.ALBUM_NAME, None) not in [None, ""]:
            data[UploadedTrackInputFieldKey.ALBUM_NAME.value] = metadata_dict.get(AppMetadataKey.ALBUM_NAME, None)
            data[UploadedTrackInputFieldKey.ALBUM_ARTISTS_NAMES.value] = metadata_dict.get(AppMetadataKey.ALBUM_ARTISTS_NAMES, [])
        return data
    ...
    def _handle_genre(self, input_data: dict, file_metadata: dict, user: User):
        ...
        if genre_name:
            ...
            input_data[UploadedTrackInputFieldKey.GENRE.value] = Genre.objects.get_or_create(user=user, name=genre_name)[0]
    ...
    def validate(self, data: dict):
        ...
        keys = [UploadedTrackInputFieldKey.TRACK_FILE_FINGERPRINT_MUST_BE_UNIQUE.value,
        ...
        if UploadedTrackInputFieldKey.ALBUM_NAME.value in data and data.get(UploadedTrackInputFieldKey.ALBUM_NAME.value) in [None, ""]:
        ...
        input_data[UploadedTrackInputFieldKey.TRACK_FILE_INTERNAL.value] = data[UploadedTrackInputFieldKey.TRACK_FILE_PUBLIC.value]
