...
class TestCase(UploadedTrackTestCase):
    ...
    def test_2_then_4(self):
        response = self._post_uploaded_track(UploadedTrackTestFilename.RATING_ID3V2_TRACKTOR_2_STAR_MP3)
        assert response.status_code == status.HTTP_201_CREATED
        assert self.saved_object.rating == 4

    def test_3_then_6(self):
        response = self._post_uploaded_track(UploadedTrackTestFilename.RATING_ID3V2_TRACKTOR_3_STAR_MP3)
        assert response.status_code == status.HTTP_201_CREATED
        assert self.saved_object.rating == 6

    def test_4_then_8(self):
        response = self._post_uploaded_track(UploadedTrackTestFilename.RATING_ID3V2_TRACKTOR_4_STAR_MP3)
        assert response.status_code == status.HTTP_201_CREATED
