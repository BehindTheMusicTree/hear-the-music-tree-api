...
class TestCase(UploadedTrackTestCase):
    ...
    def test_none_then_none_on_empty_wav(self):
        response = self._post_uploaded_track(UploadedTrackTestFilename.METADATA_NONE_WAV, **{UploadedTrackInputFieldKey.RATING.value: None})
        assert response.status_code == 201
        assert not self.saved_uploaded_track_metadata_with_raw_rating.get(AppMetadataKey.RATING, None)

    def test_none_then_none_on_empty_flac(self):
        response = self._post_uploaded_track(UploadedTrackTestFilename.METADATA_NONE_FLAC, **{UploadedTrackInputFieldKey.RATING.value: None})
        assert response.status_code == 201
        assert not self.saved_uploaded_track_metadata_with_raw_rating.get(AppMetadataKey.RATING, None)
    ...
    def test_none_then_none_on_filled_flac(self):
        response = self._post_uploaded_track(
            UploadedTrackTestFilename.METADATA_LONG_A_VORBIS_SMALL_FLAC, **{UploadedTrackInputFieldKey.RATING.value: None})
        assert response.status_code == 201
        assert not self.saved_uploaded_track_metadata_with_raw_rating.get(AppMetadataKey.RATING, None)
    ...
    def test_10_then_10_on_empty_wav(self):
        response = self._post_uploaded_track(UploadedTrackTestFilename.METADATA_NONE_WAV, **{UploadedTrackInputFieldKey.RATING.value: 10})
        assert response.status_code == 201
        assert self.saved_uploaded_track_metadata_with_raw_rating[AppMetadataKey.RATING] == 255

    def test_10_then_10_on_empty_flac(self):
        response = self._post_uploaded_track(UploadedTrackTestFilename.METADATA_NONE_FLAC, **{UploadedTrackInputFieldKey.RATING.value: 10})
        assert response.status_code == 201
