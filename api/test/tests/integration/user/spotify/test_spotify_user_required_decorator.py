...
class TestSpotifyUserRequiredDecoratorIntegration(AppTestCase):
    ...
    def test_spotify_user_retrieve_without_auth_then_401_auth_not_authenticated(self):
        self._logout()
    ...
    def test_spotify_user_retrieve_with_spotify_user_then_200_ok(self):
        ...
        data = response.json()
        assert data['id'] == self.spotify_test_user_1.pk
        assert data['spotifyId'] == self.spotify_test_user_1.spotify_id
