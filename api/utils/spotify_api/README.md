...
**Development mode:** If your app is in Development mode, only Spotify accounts added under **Settings → Users and Access** can complete OAuth. Users not in that list get a 403 from Spotify when the API calls e.g. `GET /v1/me`; the API returns 401 with code 1007 (`spotify_user_not_allowlisted`) and a message asking to add the account in the dashboard. The **Spotify account that owns the app** (the developer account) can often use the app without being in the list, which is why localhost may work with 0 users added if you are testing with the same account that created the app. On a deployed environment, other users must be added to User Management.

**Moving to non–dev mode (Extended Quota Mode):** In Extended Quota Mode, the allowlist no longer applies and an unlimited number of users can authenticate. You request it from Spotify; the API and code stay the same. In the [Developer Dashboard](https://developer.spotify.com/dashboard) open your app → **Settings** → **Quota extension Request**, complete the questionnaire and submit. Spotify reviews the app (policy compliance, etc.); the process can take several weeks. As of 2025, Extended Quota is only available to organizations (not individuals) and has requirements such as a launched service and minimum active users. See [Quota modes](https://developer.spotify.com/documentation/web-api/concepts/quota-modes) and the [application form](https://docs.google.com/forms/d/1O87xdPP1zWUDyHnduwbEFpcjA57JOaefCgBShKjAqlo) for current criteria and steps.
