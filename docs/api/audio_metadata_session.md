...
**Storage**  
Session files are stored in the env-defined directory `METADATA_SESSION_DIR`. When `TMP_UPLOADED_FILES` is set, `METADATA_SESSION_DIR` must be set (via `METADATA_SESSION_DIR_INTERNAL` or `METADATA_SESSION_DIR_EXTERNAL` in the paths script). No default; the app fails to start if `METADATA_SESSION_DIR` is missing when uploads are enabled.
