...
**Jobs:** **pre-commit** – checkout, Python 3.14, `pip install -e ".[dev]"`, `pre-commit run --all-files` (see [docs/ci/python-project-standards.md](ci/python-project-standards.md)); **check-vars-and-secrets** (Check vars and secrets) – validates required env vars and secrets; **pytest** (Pytest) – checkout → lowercase **`GHCR_IMAGE_NAMESPACE`** for Compose → **`docker login ghcr.io`** as **`GITHUB_REPOSITORY_OWNER`** + **`GITHUB_TOKEN`** (private org **`afp`** images need this repo granted **Read** under that package’s **Settings → Actions access**) → build **`api`** with Compose → pull **`db`** then **`afp`** (`postgres:16.4` and org **`AFP_*`** pins from [`docker-compose.yml`](docker-compose.yml)) → **`docker compose up --wait`** for **`db`** and **`afp`** → **`docker compose run api`** (setup filesystem, wait for Postgres and AFP, init Django data, pytest with JUnit XML on the workspace mount) → publish test results → Compose teardown.
