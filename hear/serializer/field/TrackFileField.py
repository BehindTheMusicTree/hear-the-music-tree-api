...
from hear.serializer.field.AppFileField import AppFileField
from hear.serializer.field.AppUrlField import AppUrlField
from hear.validator.TrackFileValidator import TrackFileValidator
from hear.validator.TrackUrlValidator import TrackUrlValidator
    ...
    def _download_file_from_url(self, url: str) -> TemporaryUploadedFile:
        ...
        try:
            ...
            temp_file = TemporaryUploadedFile(
                name=filename, content_type=response.headers.get("Content-Type", ""), size=file_size, charset=None
            )
            ...
            raise AppValidationException(
                field_name=self.get_error_field_name(),
                message=f"Unexpected error while downloading file: {e!s} ",
                field_validation_error_code=FieldValidationErrorCode.TRACK_FILE_DOWNLOAD_FAILED,
            )
