...
if CiStartupTraceEnabled.is_tracer_active():
    print("[pytest] hear/test/tests/conftest.py: import line 1 (stdlib only; next imports may block)", flush=True)
...
if _pytest_startup_progress():
    print(
        "[pytest] hear/test/tests/conftest.py: top-level imports finished (django.setup may already have run).",
        flush=True,
    )
...
def mock_oauth_outside_e2e(request):
    ...
    if not _should_mock_external_services(request):
        yield
        return
    with patch("hear.view.spotify_auth.SpotifyOAuthService"), patch("hear.view.google_auth.GoogleOAuthService"):
        yield
...
def mock_musicbrainz_outside_e2e(request):
    ...
    if request.node.get_closest_marker("patches_musicbrainz_lookup"):
        ...
        return
    with patch(
        "hear.utils.musicbrainz.service.acoustid.lookup",
...
def mock_spotify_client_outside_e2e(request):
    ...
    if _should_mock_external_services(request):
        ...
        with (
            ...
            patch(
                "hear.utils.spotify_api.managers.SpotifyApiArtistManager.get_spotify_client",
                return_value=mock_instance,
            ),
            patch(
                "hear.view.viewset.model.SpotifyArtistViewSet.get_spotify_client",
                return_value=mock_instance,
            ),
        ):
            yield
...
def _make_success_fingerprinting_result():
    from hear.model.uploaded_track.file.fingerprinting.FingerprintingResult import FingerprintingResult
...
def mock_audio_meta_analysis_outside_e2e(request):
    ...
    with patch(
        "hear.utils.audio_fingerprinter.service.get_fingerprinting_result",
