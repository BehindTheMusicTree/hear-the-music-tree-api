...
class TestCase(GenreTestCase):
    def test_new_root_then_update_root_of_descendants(self):
        ...
        frenchhardcore_genre = self.model_fixture_factory.create_genre(
            name="French hardcore", parent=punkhardcore_genre
        )
        ...
        updated_genre_punk_playlist: CriteriaPlaylist = CriteriaPlaylist.objects.get(
            user=self.test_user1, criteria=genre_punk
        )
        assert updated_genre_punk_playlist.root == root_playlist

        updated_punkhardcore_genre_playlist: CriteriaPlaylist = CriteriaPlaylist.objects.get(
            user=self.test_user1, criteria=punkhardcore_genre
        )
        assert updated_punkhardcore_genre_playlist.root == root_playlist

        updated_frenchhardcore_genre_playlist: CriteriaPlaylist = CriteriaPlaylist.objects.get(
            user=self.test_user1, criteria=frenchhardcore_genre
        )
