...
from hear.test.utils.field.body_data.type.NotNullableCharBodyDataTestCase import NotNullableCharBodyDataTestCase
...
class TestCase(NotNullableCharBodyDataTestCase, UploadedTrackTestCase):
    def test_largest_then_ok(self):
        ...
        response = self._post_uploaded_track(
            UploadedTrackTestFilename.METADATA_NONE_MP3, **{UploadedTrackInputFieldKey.TITLE.value: value}
        )
    ...
    def test_empty_then_400_bad_request(self):
        response = self._post_uploaded_track(
            UploadedTrackTestFilename.METADATA_NONE_MP3, **{UploadedTrackInputFieldKey.TITLE.value: ""}
        )
