from rest_framework import status
from the_music_tree_api_kit.exception.validation.FieldValidationErrorCode import FieldValidationErrorCode

from hear.test.tests.integration.uploaded_track.UploadedTrackTestCase import UploadedTrackTestCase


class TestCase(UploadedTrackTestCase):
    def test_filter_not_existing_then_400_bad_request(self):
        invalid_filter = "invalidFilter"
        response = self._list_uploaded_tracks(**{invalid_filter: "invalid_filter"})

        assert response.status_code == status.HTTP_400_BAD_REQUEST
        assert self.bad_request_result_field_errors[0]["field"] == invalid_filter
        assert self.bad_request_result_field_errors[0]["code"] == FieldValidationErrorCode.INVALID_FILTER
