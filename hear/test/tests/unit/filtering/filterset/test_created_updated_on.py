...
class TestPrivateUniqueResourceFilterSet(AppTestCase):
    ...
    def test_created_on_lt_filter(self):
        filterset = PrivateUniqueResourceFilterSet(
            {PrivateUniqueResourceFields.CREATED_ON_LT: self.now.isoformat()}, queryset=Genre.objects.all()
        )
        assert list(filterset.qs) == [self.past_record]

    def test_created_on_gte_filter(self):
        filterset = PrivateUniqueResourceFilterSet(
            {PrivateUniqueResourceFields.CREATED_ON_GTE: self.now.isoformat()}, queryset=Genre.objects.all()
        )
        assert set(filterset.qs) == {self.present_record, self.future_record}

    def test_created_on_lte_filter(self):
        filterset = PrivateUniqueResourceFilterSet(
            {PrivateUniqueResourceFields.CREATED_ON_LTE: self.now.isoformat()}, queryset=Genre.objects.all()
        )
        assert set(filterset.qs) == {self.past_record, self.present_record}

    def test_updated_on_exact_filter(self):
        filterset = PrivateUniqueResourceFilterSet(
            {PrivateUniqueResourceFields.UPDATED_ON: self.now.isoformat()}, queryset=Genre.objects.all()
        )
        assert list(filterset.qs) == [self.present_record]

    def test_updated_on_gt_filter(self):
        filterset = PrivateUniqueResourceFilterSet(
            {PrivateUniqueResourceFields.UPDATED_ON_GT: self.now.isoformat()}, queryset=Genre.objects.all()
        )
        assert list(filterset.qs) == [self.future_record]

    def test_updated_on_lt_filter(self):
        filterset = PrivateUniqueResourceFilterSet(
            {PrivateUniqueResourceFields.UPDATED_ON_LT: self.now.isoformat()}, queryset=Genre.objects.all()
        )
        assert list(filterset.qs) == [self.past_record]

    def test_updated_on_gte_filter(self):
        filterset = PrivateUniqueResourceFilterSet(
            {PrivateUniqueResourceFields.UPDATED_ON_GTE: self.now.isoformat()}, queryset=Genre.objects.all()
        )
        assert set(filterset.qs) == {self.present_record, self.future_record}

    def test_updated_on_lte_filter(self):
        filterset = PrivateUniqueResourceFilterSet(
            {PrivateUniqueResourceFields.UPDATED_ON_LTE: self.now.isoformat()}, queryset=Genre.objects.all()
        )
        assert set(filterset.qs) == {self.past_record, self.present_record}

    def test_created_on_invalid_datetime_format(self):
        filterset = PrivateUniqueResourceFilterSet(
            {PrivateUniqueResourceFields.CREATED_ON: "invalid-datetime"}, queryset=Genre.objects.all()
        )
        assert not filterset.is_valid()

    def test_created_on_gt_invalid_datetime_format(self):
        filterset = PrivateUniqueResourceFilterSet(
            {PrivateUniqueResourceFields.CREATED_ON_GT: "invalid-datetime"}, queryset=Genre.objects.all()
        )
        assert not filterset.is_valid()

    def test_created_on_gte_invalid_datetime_format(self):
        filterset = PrivateUniqueResourceFilterSet(
            {PrivateUniqueResourceFields.CREATED_ON_GTE: "invalid-datetime"}, queryset=Genre.objects.all()
        )
        assert not filterset.is_valid()

    def test_created_on_lt_invalid_datetime_format(self):
        filterset = PrivateUniqueResourceFilterSet(
            {PrivateUniqueResourceFields.CREATED_ON_LT: "invalid-datetime"}, queryset=Genre.objects.all()
        )
        assert not filterset.is_valid()

    def test_created_on_lte_invalid_datetime_format(self):
        filterset = PrivateUniqueResourceFilterSet(
            {PrivateUniqueResourceFields.CREATED_ON_LTE: "invalid-datetime"}, queryset=Genre.objects.all()
        )
        assert not filterset.is_valid()

    def test_updated_on_invalid_datetime_format(self):
        filterset = PrivateUniqueResourceFilterSet(
            {PrivateUniqueResourceFields.UPDATED_ON: "invalid-datetime"}, queryset=Genre.objects.all()
        )
        assert not filterset.is_valid()

    def test_updated_on_gt_invalid_datetime_format(self):
        filterset = PrivateUniqueResourceFilterSet(
            {PrivateUniqueResourceFields.UPDATED_ON_GT: "invalid-datetime"}, queryset=Genre.objects.all()
        )
        assert not filterset.is_valid()

    def test_updated_on_gte_invalid_datetime_format(self):
        filterset = PrivateUniqueResourceFilterSet(
            {PrivateUniqueResourceFields.UPDATED_ON_GTE: "invalid-datetime"}, queryset=Genre.objects.all()
        )
        assert not filterset.is_valid()

    def test_updated_on_lt_invalid_datetime_format(self):
        filterset = PrivateUniqueResourceFilterSet(
            {PrivateUniqueResourceFields.UPDATED_ON_LT: "invalid-datetime"}, queryset=Genre.objects.all()
        )
        assert not filterset.is_valid()

    def test_updated_on_lte_invalid_datetime_format(self):
        filterset = PrivateUniqueResourceFilterSet(
            {PrivateUniqueResourceFields.UPDATED_ON_LTE: "invalid-datetime"}, queryset=Genre.objects.all()
        )
        assert not filterset.is_valid()
