...
from hear.exception import musicbrainz as musicbrainz_exception
from hear.model.musicbrainz_resource.children.recording.MbRecording import MbRecording
from hear.model.musicbrainz_resource.children.recording.MbRecordingLookupResult import MusicbrainzRecordingLookupResult
    ...
    try:
        ...
        if lookup_status == ApiFields.Values.Status.OK:
            ...
            if len(recordings_grouped_by_score) > 0:
                return utils.get_best_recording_dict_with_score(
...
def get_musicbrainz_recording_lookup_result(
    user: User, fingerprint: bytes, duration_in_sec: float
    ...
    if duration_in_sec <= 1:
        ...
        try:
            ...
            exception_mapping: dict[type, MbRecordingMissingCauseCode.Codes] = {
                musicbrainz_exception.InvalidFingerprintMusicbrainzRecordingLookupException: MbRecordingMissingCauseCode.Codes.LOOKUP_FAILED_DUE_TO_INVALID_FINGERPRINT,
                musicbrainz_exception.InternalErrorMusicbrainzRecordingLookupException: MbRecordingMissingCauseCode.Codes.LOOKUP_FAILED_WITH_INTERNAL_ERROR,
                musicbrainz_exception.UnknownErrorCodeMusicbrainzRecordingLookupException: MbRecordingMissingCauseCode.Codes.LOOKUP_FAILED_WITH_UNKNOWN_RESPONSE_ERROR_CODE,
                musicbrainz_exception.UnknownStatusMusicbrainzRecordingLookupException: MbRecordingMissingCauseCode.Codes.LOOKUP_FAILED_WITH_UNKNOWN_RESPONSE_STATUS_CODE,
                musicbrainz_exception.DNSResolutionErrorMusicbrainzRecordingLookupException: MbRecordingMissingCauseCode.Codes.LOOKUP_FAILED_DNS_RESOLUTION_ERROR,
            }
            ...
            if len(error_message) > settings.MB_RECORDING_MISSING_CAUSE_MESSAGE_LEN_MAX:
                musicbrainz_recording_missing_cause_message = (
                    error_message[: settings.MB_RECORDING_MISSING_CAUSE_MESSAGE_LEN_MAX - 3] + "..."
                )
            else:
    ...
    return MusicbrainzRecordingLookupResult(
        recording=musicbrainz_recording, missing_cause=musicbrainz_recording_missing_cause
    )
