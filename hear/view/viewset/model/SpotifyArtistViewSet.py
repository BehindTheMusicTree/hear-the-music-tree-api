...
from hear.filtering.set.spotify.artist.Fields import Fields as FilterFields
from hear.filtering.set.spotify.artist.SpotifyArtistFilterSet import SpotifyArtistFilterSet
from hear.model.spotify_resource.children.artist.SpotifyArtist import SpotifyArtist
from hear.serializer.model.spotify.artist.output.detailed import SpotifyArtistDetailedSerializer
from hear.serializer.model.spotify.artist.output.simple import SpotifyArtistSimpleSerializer
from hear.utils.spotify_api.SpotifyClient import get_spotify_client
...
class SpotifyArtistViewSet(AppModelViewSet[SpotifyArtist]):
    ...
    def list(self, *args, **kwargs):
        if self.spotify_client is None:
            return Response({"detail": "Spotify is not configured."}, status=status.HTTP_503_SERVICE_UNAVAILABLE)
