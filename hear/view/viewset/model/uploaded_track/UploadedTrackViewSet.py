...
from hear.filtering.set.uploaded_track.UploadedTrackFilterFieldKey import UploadedTrackFilterFieldKey
from hear.filtering.set.uploaded_track.UploadedTrackFilterSet import UploadedTrackFilterSet
from hear.model.uploaded_track.UploadedTrack import UploadedTrack
from hear.serializer.model.uploaded_track.input.post.post import UploadedTrackPostSerializer
from hear.serializer.model.uploaded_track.input.put.put import UploadedTrackPutSerializer
...
class UploadedTrackViewSet(AppModelViewSet[UploadedTrack]):
    ...
    @extend_schema(
        request=UploadedTrackPostSerializer,
    ...
    def create(self, request, *args, **kwargs):
        try:
            ...
            if request.FILES.get(UploadedTrackInputFieldKey.TRACK_FILE_PUBLIC.value) and hasattr(
                request.FILES[UploadedTrackInputFieldKey.TRACK_FILE_PUBLIC.value], "temporary_file_path"
            ):
                try:
    ...
    @extend_schema(
        parameters=[
            OpenApiParameter(
                name=UploadedTrackFilterFieldKey.TITLE.value, type=OpenApiTypes.STR, location=OpenApiParameter.QUERY
            ),
            OpenApiParameter(
                name=UploadedTrackFilterFieldKey.ARTISTS_NAME.value,
                type=OpenApiTypes.STR,
                location=OpenApiParameter.QUERY,
            ),
            OpenApiParameter(
                name=UploadedTrackFilterFieldKey.ALBUM_NAME.value,
                type=OpenApiTypes.STR,
                location=OpenApiParameter.QUERY,
            ),
            OpenApiParameter(
                name=UploadedTrackFilterFieldKey.GENRE_NAME.value,
                type=OpenApiTypes.STR,
                location=OpenApiParameter.QUERY,
            ),
            OpenApiParameter(
                name=UploadedTrackFilterFieldKey.LANGUAGE.value, type=OpenApiTypes.STR, location=OpenApiParameter.QUERY
            ),
        ]
    )
    ...
    @extend_schema(
        request=UploadedTrackPutSerializer,
        description=(
            """
            Updates a track:\n"
        ...
        responses={
            ...
            400: OpenApiResponse(
                description="Validation error. Response body contains fieldErrors (field, code, message)."
            ),
        },
    )
